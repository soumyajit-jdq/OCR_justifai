...
class KeyValueField(BaseModel):
    ...
    value: str = Field(description="The extracted value for this field. Can be in English, Gujarati, or Hindi.")

class TableRow(BaseModel):
    cells: List[KeyValueField] = Field(description="The cells of the row. Use the column name as the 'key' and the cell contents as the 'value'. Values can be English, Gujarati, or Hindi.")
...
class GenericDocumentData(BaseModel):
    ...
    main_details: List[KeyValueField] = Field(description="General key-value information extracted from the document. Supports English, Gujarati, and Hindi values.")
    tables: List[DataTable] = Field(description="Any structured or tabular data found in the document. Supports English, Gujarati, and Hindi content.")
...
def parse_with_gemini(image_path: str, api_key: str, max_retries: int = 4) -> str:
    """Uses Google's Gemini API with native Vision capabilities and Structured Outputs (Trilingual)"""
    if not HAS_GEMINI:
    ...
    print("\nSending image directly to Gemini API for Trilingual processing (English + Gujarati + Hindi)...")
    ...
    prompt = (
        ...
        "The document may contain text in English, Gujarati, and Hindi. "
        "Extract every single field exactly as it appears, preserving the original language (Hindi, Gujarati, or English). "
...
def parse_with_openai(image_path: str, api_key: str) -> str:
    """Uses OpenAI's API with native Vision capabilities and Structured Outputs (Trilingual)"""
    if not HAS_OPENAI:
        raise ImportError("openai package is not installed. Run `pip install openai`")
    
    print("\nSending image directly to OpenAI API for Trilingual processing (English + Gujarati + Hindi)...")
    ...
    completion = client.beta.chat.completions.parse(
        model="gpt-4o",
        messages=[
            {"role": "system", "content": "You are a helpful assistant that parses document images into a structured JSON format. You support multiple languages including English, Gujarati, and Hindi."},
            {"role": "user", "content": [
...
def main():
    parser = argparse.ArgumentParser(description="Trilingual (Eng+Guj+Hin) Parser using AI Native Vision")
    ...
    print(f"Analyzing Trilingual Image: {args.image_path}")
    ...
    try:
        ...
        print("\n" + "="*60)
        print("TRILINGUAL STRUCTURED JSON RESULT (Eng + Guj + Hin)")
        print("="*60)
        ...
        output_file = os.path.join(output_dir, f"{base_name}_trilingual_parsed.json")
        
        with open(output_file, 'w', encoding='utf-8') as f:
            json.dump(structured_data, f, indent=2, ensure_ascii=False)
            
        print(f"\nSuccessfully saved trilingual parsed data to {output_file}")
        
    except Exception as e:
        print(f"\nError during Trilingual Parsing: {str(e)}")
